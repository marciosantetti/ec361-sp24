...
saving_ts |> 
  autoplot(difference(psav, lag = 1)) +
  labs(title = "U.S. personal saving rate: First differences",
       y = "",
       subtitle = "1959–2023",
       caption = "Source: U.S. Bureau of Economic Analysis.") +
  easy_y_axis_title_size(13) +
...
saving_arima_fit <- saving_ts |> 
  model(arima012 = ARIMA(psav ~ pdq(0, 1, 2)), # "1 + pdq()" includes a constant (c) if needed.
        arima210 = ARIMA(psav ~ pdq(2, 1, 0)), # "1 + pdq()" includes a constant (c) if needed.
...
saving_arima_fc |> 
  filter(.model == "arima210") |> 
  autoplot(saving_ts, level = 95) +
  labs(title = "6-year ahead forecast: ARIMA(2, 1, 0) model",
       subtitle = "U.S. personal saving rate",
       y = "Percent",
       x = "") +
  easy_y_axis_title_size(13)
